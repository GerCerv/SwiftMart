                                ...
                                <div class="relative inline-block">
                                    @if($product->discount > 0)
                                        <div class="absolute -top-6 -right-7 transform rotate-12 border border-pink-700 bg-pink-600 text-white text-xs font-extrabold px-2 py-1 shadow-lg rounded-tr-md z-100 transition-transform duration-300 hover:scale-110">
                                            ...
                                            @if ($product->hotDeal)
                                               🔥 {{ $product->hotDeal->discount_percentage + $product->discount }}% OFF
                                            @else
                                        ...
                                        <div class="flex flex-col items-end">
                                            <span class="text-green-700 font-bold text-3xl">
                                                ₱{{ number_format($product->price - ($product->price * (($product->discount + $product->hotDeal->discount_percentage)  / 100)), 2) }}
                                            </span>
                                    ...
                                    @else
                                        @if ($product->hotDeal)
                                        <div class="absolute -top-6 -right-7 transform rotate-12 border border-pink-700 bg-pink-600 text-white text-xs font-extrabold px-2 py-1 shadow-lg rounded-tr-md z-100 transition-transform duration-300 hover:scale-110">
                                            🔥 {{ $product->hotDeal->discount_percentage}}% OFF
                                        </div>
                                        <div class="flex flex-col items-end">
                                            <span class="text-green-700 font-bold text-3xl">
                                                ₱{{ number_format($product->price - ($product->price * ($product->hotDeal->discount_percentage / 100)), 2) }}
                                            </span>
                                            <small class="text-gray-500 line-through text-sm">
                                                ₱{{ number_format($product->price, 2) }}
                                            </small>
            ...
            @foreach($products as $product)
                <div class="bg-white rounded-lg shadow-md overflow-hidden relative">
                    <div class="relative h-32 bg-gray-200">
                        <img src="{{ $product->image ? asset('storage/products/' . $product->image) : asset('images/emp.jpg') }}" alt="{{ $product->name }}" class="w-full h-full object-cover">
                        @if($product->discount > 0)
                        ...
                        <div class="absolute top-1 right-1 z-10">
                            @auth
                                <button class="wishlist-btn p-1 rounded-full bg-white shadow-md {{ auth()->user()->hasInWishlist($product->id) ? 'text-red-500' : 'text-gray-500' }}"
                                        data-product-id="{{ $product->id }}"
                                        data-product-name="{{ $product->name }}"
                                        data-product-price="{{ number_format($product->price, 2, '.', '') }}"
                                        data-product-image="{{ $product->image ? asset('storage/products/' . $product->image) : asset('images/emp.jpg') }}"
                                        title="{{ auth()->user()->hasInWishlist($product->id) ? 'Remove from Wishlist' : 'Add to Wishlist' }}"
                                        data-product-stock="{{ $product->stock }}">
                                    <i class="fa-heart {{ auth()->user()->hasInWishlist($product->id) ? 'fas' : 'far' }} text-sm"></i>
                                </button>
                            @else
                                <a href="{{ route('login') }}" class="wishlist-btn p-1 rounded-full bg-white text-gray-500 shadow-md"
                                title="Login to add to wishlist">
                    ...
                    <div class="p-2">
                        <h3 class="text-sm font-semibold text-green-900">{{ ucfirst($product->name) }}</h3>
                        <p class="text-xs text-green-700"><i class="fas fa-store"></i> {{ $product->vendor->store_name ?? 'Unknown' }}</p>
                        <div class="flex text-yellow-400">
                            @for($i = 1; $i <= 5; $i++)
                                <i class="fas fa-star {{ $product->averageRating() >= $i ? 'text-yellow-400' : 'text-gray-300' }}"></i>
                            @endfor
                        </div>
                        <div class="flex items-baseline space-x-1 mt-1">
                            @if($product->discount > 0)
                                @if ($product->hotDeal)
                                <span class="text-base font-bold text-green-700">₱{{ number_format($product->price - 
                                    ($product->price * (($product->discount + $product->hotDeal->discount_percentage ) / 100)), 2) }}</span>
                                <span class="text-xs text-gray-500 line-through">₱{{ number_format($product->price, 2) }}</span>
                                @else
                                <span class="text-base font-bold text-green-700">₱{{ number_format($product->price - ($product->price * ($product->discount / 100)), 2) }}</span>
                                <span class="text-xs text-gray-500 line-through">₱{{ number_format($product->price, 2) }}</span>
                                @endif
                            ...
                            @else
                                @if ($product->hotDeal)
                                <span class="text-base font-bold text-green-700">₱{{ number_format($product->price - 
                                    ($product->price * (($product->hotDeal->discount_percentage ) / 100)), 2) }}</span>
                                <span class="text-xs text-gray-500 line-through">₱{{ number_format($product->price, 2) }}</span>
                                @else
                                <span class="text-base font-bold text-green-700">₱{{ number_format($product->price, 2) }}</span>
                                @endif
                                
                            @endif
                        </div>
                        <button class="add-to-cart-btn btn-primary text-white px-2 py-1 rounded-lg text-xs mt-1 w-full" data-product-id="{{ $product->id }}" {{ $product->stock < 1 ? 'disabled' : '' }}>
                            <i class="fas fa-shopping-basket mr-1"></i> Add
                        </button>
                    </div>
                </div>
            @endforeach
