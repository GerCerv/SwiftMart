...
<div class="d-flex flex-column">
    ...
    <div class="main-content flex-grow-1">
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $user->delivered_orders_count ?? 0 }}</td>
                                    <td>
                                        <span class="badge bg-{{ $user->complaints_count > 0 ? 'danger' : 'secondary' }}">
                                    ...
                                    <td>
                                        @if($user->complaints_count > 0)
                                            <button class="btn btn-sm btn-primary"
                                                    ...
                                                    data-bs-target="#complaintsModal{{ $user->id }}">
                                                <i class="fas fa-comments"></i> View Complaints
                                            </button>
                                        @else
                                            <span class="text-muted">No Complaints</span>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
        ...
        <!-- Complaints Modals (moved outside table) -->
        @foreach ($users as $user)
            @if($user->complaints_count > 0)
            <div class="modal fade" id="complaintsModal{{ $user->id }}" tabindex="-1"
                aria-labelledby="complaintsModalLabel{{ $user->id }}" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="complaintsModalLabel{{ $user->id }}">Complaints for {{ $user->name }}</h5>
                                    ...
                                    <li class="list-group-item">
                                        <div class="d-flex justify-content-between">
                                            @if ($complaint->product)
                                                <strong>#{{ $complaint->product_id }} : {{ $complaint->product->name }}</strong>
                                            @endif
                                            <small class="text-muted">{{ $complaint->created_at->format('d/m/Y') }}</small>
                                        </div>
                                        <div>
                                            <small class="text-muted">{{ ucfirst($complaint->store_name) }}: {{ Str::limit($complaint->message, 50) }}</small>
                                        </div>
                                    </li>
                                @endforeach
                                </ul>
