                            ...
                            <tbody>
                                @foreach ($Orders as $order)
                                    <tr>
                                        ...
                                        <td>
                                            @foreach ($order->items->pluck('vendor.name')->unique() as $vendorName)
                                        ...
                                        <td>
                                            ...
                                            <!-- <form action="#" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-danger delete-order" 
    ...
    $('.view-order').on('click', function() {
        try {
            ...
            if (!items || !Array.isArray(items)) {
                ...
                $('#orderDetails').html('<tr><td colspan="6">Error loading items</td></tr>');
                return;
            }

            items.forEach(item => {
                const imageUrl = item.product && item.product.image 
                    ? `/storage/products/${item.product.image}` 
                    : '/images/placeholder.png'; // fallback placeholder image

                const image = `<img src="${imageUrl}" alt="${item.product?.name || 'Product'}" style="width: 150px; height: auto;">`;
                const productName = item.product?.name || 'Unknown Product';
                const price = item.unit_price ? Number(item.unit_price).toFixed(2) : 'N/A';
                const packageSize = item.pack_size || 'N/A';
            ...
            $('#orderModalLabel').text(`Order Details - Order #${orderId}`);
            $('#orderDetails').html(html);
        } catch (error) {
            console.error('Error rendering modal content:', error);
            $('#orderDetails').html('<tr><td colspan="6">Error loading order details</td></tr>');
        }
    });
});
</script>
