...
<div class="d-flex flex-column">
    ...
    <div class="main-content flex-grow-1">
       <div class="container-fluid">
                            ...
                            <tbody>
                                @foreach($allproducts as $all)
                                    <tr>
                                        ...
                                        <td>
                                            {{ $all->vendor->name }}
                                            <small class="d-block text-muted">{{ $all->vendor->store_name }}</small>
                                        </td>
                                        <td>{{ $all->name }}</td>
                                        <td>{{ $all->category }}</td>
                                        <td>₱{{ number_format($all->price, 2) }}</td>
                                        <td>{{ $all->stock }}</td>
                                        <td>{{ $all->discount }}%</td>
                                        ...
                                        <td class="d-flex gap-2">
                                            ...
                                            <button class="btn btn-sm btn-info" data-bs-toggle="modal" 
                                                    data-bs-target="#imagesModal-{{ $all->id }}">
        ...
        @foreach($allproducts as $all)
            <div class="modal fade" id="imagesModal-{{ $all->id }}" tabindex="-1" 
                aria-labelledby="imagesModalLabel-{{ $all->id }}" aria-hidden="true" style="z-index: 1056;">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">{{ $all->name }} Images</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" 
                                    aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div id="carousel-{{ $all->id }}" class="carousel slide" data-bs-ride="carousel">
                                ...
                                @if(count($images) > 1)
                                    <button class="carousel-control-prev" type="button" 
                                            data-bs-target="#carousel-{{ $all->id }}" data-bs-slide="prev">
                                        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                        <span class="visually-hidden">Previous</span>
                                    </button>
                                    <button class="carousel-control-next" type="button" 
                                            data-bs-target="#carousel-{{ $all->id }}" data-bs-slide="next">
                        ...
                        <div class="modal-footer">
                            <div class="d-flex justify-content-between w-100">
                                <span class="text-muted">
                                    {{ count($images) }} image(s) available
                                </span>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    deleteModal.addEventListener('show.bs.modal', function (event) {
        ...
        const routeTemplate = "{{ route('admin.products.delete', ':id') }}";
