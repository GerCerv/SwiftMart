                            ...
                            <tbody id="vendors-table-body">
                                @forelse ($vendors as $vendor)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge 
                                                {{ $vendor->status === 'pending' ? 'bg-secondary' : 'bg-success' }}">
                                                {{ ucfirst($vendor->status) }}
                                            </span>
                                        </td>
                                        <td>
                                            <form action="{{ route('vendors.updateStatus', $vendor->vendor_id) }}" method="POST" style="display:inline-block; margin-left:5px;">
                                                @csrf
                                                @method('PATCH')
                                                @if($vendor->status === 'pending')
                                                    <button type="submit" name="status" value="approved" class="btn btn-success btn-sm">Approve</button>
                                                @elseif($vendor->status === 'approved')
                                                    <button type="submit" name="status" value="suspended" class="btn btn-warning btn-sm">Suspended</button>
                                                @elseif($vendor->status === 'suspended')
                                                    <button type="submit" name="status" value="approved" class="btn btn-warning btn-sm">Unsuspend</button>
                                                    
                                                @endif
                                            </form>

                                            <form action="{{ route('vendors.destroy', $vendor->vendor_id) }}" method="POST" style="display:inline-block; margin-left:5px;">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center">No vendors available.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
